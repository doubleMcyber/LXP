...
def test_mac_mps_stage2_smoke_command_can_eval_on_train() -> None:
    ...
    assert "training.lambda_answer=40.0" in command
    assert "training.lambda_answer_first_token=80.0" in command
    assert "training.lambda_logit_steering=120.0" in command
    ...
    assert "training.latent_logit_steering.rank=128" in command
    assert "training.latent_logit_steering.vocabulary_mode=low_rank" in command
    assert "training.latent_logit_steering.lr_multiplier=10.0" in command
    assert "training.latent_logit_steering.output_steps=1" in command
