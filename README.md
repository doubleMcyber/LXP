...
- Cross-family latent continuation works, N=8: LGAI-EXAONE/EXAONE-4.0-1.2B -> Qwen/Qwen3.5-2B mid-reasoning continuation at truncation 0.5 reaches latent 87.5% (7/8) versus text 62.5% versus receiver-alone 37.5%, using the cached 32-row cross-family linear ridge adapter with no receiver fine-tuning. Caveat: N=8, one truncation fraction, small-drafter to big-finisher direction.
- Truncation dose-response (N=32 per point, audited path, per-fraction leak-free adapters): at f=0.25 latent 75.0% vs text-hybrid 62.5%; at f=0.5 latent 65.6% vs text-hybrid 56.2%. The per-point latent leads are directional; the N=128 run above is what anchors significance.
