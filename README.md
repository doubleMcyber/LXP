...
- Mid-reasoning latent continuation now beats text handoff at significance through the audited benchmark path. On GSM8K validation with `generated_context_latent_handoff` at truncation 0.5 (locked manifest, leak-free, Qwen/Qwen3.5-2B -> Qwen/Qwen3.5-2B), latent reaches 68.8% (88/128) versus text-hybrid 57.0% versus receiver-alone 14.1% at N=128. The paired per-row McNemar exact test gives p = 0.0041 for latent vs text and p about 2e-18 for latent vs alone. The advantage holds on the copy-proof stratum where the answer literal is absent from the truncated sender text (68.9% vs 57.5%, 106 rows), and the latent handoff is faster per handoff than the text hybrid (14.4s vs 18.5s). Mid-reasoning continuation runs use `--methods generated_context_latent_handoff` (the receiver-context variant); the latent-only method is not a continuation test.
- Cross-family latent continuation carries the computation at parity with text, N=32: LGAI-EXAONE/EXAONE-4.0-1.2B -> Qwen/Qwen3.5-2B mid-reasoning continuation at truncation 0.5 (locked manifest, leak-free) reaches latent 59.4% (19/32) versus text-hybrid 62.5% (20/32) — a statistical tie (McNemar p=1.0, exact tie on the copy-proof stratum) — with both far above receiver-alone 21.9% (latent vs alone p=0.0018). The latent handoff is about 40% faster than the text hybrid (13.1s vs 21.5s per sample) at equal fidelity. A 32-row cross-family linear ridge adapter, no receiver fine-tuning; the latent advantage over text is so far a same-family result, and adapter density (32 vs 128 training rows) is the untested lever.
- Truncation dose-response (N=32 per point, audited path, per-fraction leak-free adapters): at f=0.25 latent 75.0% vs text-hybrid 62.5%; at f=0.5 latent 65.6% vs text-hybrid 56.2%; at f=0.75 latent 78.1% vs text-hybrid 56.2% (7 latent-only wins vs 0 text-only, McNemar p=0.016 — significant even at N=32). The f=0.25/f=0.5 per-point leads are directional; the N=128 run above and the f=0.75 point anchor significance.
- GSM8K also has a generated-reasoning latent handoff readout path: the sender produces novel reasoning traces, the receiver consumes a 12-step latent prefix, and the Qwen/Qwen3.5-2B -> Qwen/Qwen3.5-0.8B bfloat16 run reaches 100% (8/8). The fast readout run uses `final_answer_tail_anchored`, `tail_tokens`, and one global `ridge` adapter; the receiver-forward decode path with token readout disabled has also been validated at 100% (8/8) with answer perplexity about 1.65.
...
- The long-context ladder was re-certified end-to-end after the 2026-07 continuation-parity fixes (fused single-forward receiver-context prefix, truncation-aware latent answer suffix, and the `logits_to_keep` memory fix): all methods 100% and all semantic, comparison, and heterogeneous gates pass.
