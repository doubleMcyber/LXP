...
- Cross-family latent continuation beats text at N=128: LGAI-EXAONE/EXAONE-4.0-1.2B -> Qwen/Qwen3.5-2B mid-reasoning continuation at truncation 0.5 (locked manifest `outputs/paths/locked_xfam128sig.json`, leak-free) reaches latent 78.1% (100/128) versus text-hybrid 64.1% (82/128), McNemar exact p = 0.0064 (copy-proof stratum 79.0% vs 64.7% over 119 rows, p = 0.0095), both far above receiver-alone 14.1%. A 128-row cross-family linear ridge adapter, no receiver fine-tuning. Adapter density is the lever: the same protocol with a 32-row adapter was a statistical tie (59.4% vs 62.5%, p=1.0). In fully warm-cache conditions the cross-family latent handoff is also about 40% faster than the text hybrid (13.1s vs 21.5s per sample, N=32 run).
